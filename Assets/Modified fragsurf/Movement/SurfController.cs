...
namespace Fragsurf.Movement {
    public class SurfController {
        ...
        private Vector3 grappleJumpVelocity;

        public void JumpToPosition(Vector3 targetPosition, float trajectoryHeight) {
            activeGrapple = true;

            //grappleJumpVelocity = CalculateJumpVelocity(playerTransform.position, targetPosition, trajectoryHeight);
            //Invoke(nameof(SetGrappleJumpVelocity), 0.1f);
            _surfer.moveData.velocity = CalculateJumpVelocity(playerTransform.position, targetPosition, trajectoryHeight);
        }

        private void SetGrappleJumpVelocity() {
            _surfer.moveData.velocity = grappleJumpVelocity;
        }
